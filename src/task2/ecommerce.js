...
export const fetchAllOrders = async () => {
    ...
    const orders = allIds.map(async (id) => {
...
export const getLast2WeeksOrders = async () => {
    ...
    // Explain Solution: 1000[ms] * (60[s] * 60[m] * 24[h]) * 14[d] saved in exponential notation.
    const twoWeeksAgo = Date.now() - 1000 * 60 * 60 * 24 * 14;
    const orders = await fetchAllOrders();
    const maxTwoWeeksOrders = orders.filter((order) => {
        return order.timestamp > twoWeeksAgo;
    });
    return maxTwoWeeksOrders;
};
